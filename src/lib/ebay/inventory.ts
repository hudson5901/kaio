import { getBaseApiUrl, getApplicationToken } from "./auth";
...
export async function createEbayListing(item: Item): Promise<{
  ...
  const engDescription = generateEnglishDescription(item.mercariTitle, description, dimensions);
  ...
  const inventoryItem = {
    ...
    product: {
      ...
      imageUrls: processedImages.length > 0
        ? processedImages.map((p) => `${process.env.NEXT_PUBLIC_BASE_URL || "http://localhost:3000"}${p}`)
        : [],
      aspects: {
        Type: ["Katana"],
